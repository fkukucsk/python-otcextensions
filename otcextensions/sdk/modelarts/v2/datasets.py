...
class Sample(resource.Resource):
    ...
    #:  http:// indicates the link address, and the value
    #:  starting with content:// indicates the text.
...
class Label(resource.Resource):
    ...
    #:  If this parameter is not passed, the current dataset type is used
    #:  by default.
...
class Datasets(resource.Resource):
    base_path = '/datasets'
    ...
    #: Input dataset path, which is used to synchronize source data
    #:  (such as images, text files, and audio files) in the directory
    #:  and its subdirectories to the dataset. config array
    #:  corresponding to infer_type
    data_sources = resource.Body(
        'data_sources',
        type=list,
        list_type=DataSource)
    ...
    #: This field is mandatory for text datasets (such as datasets of
    ...
    #: Whether to synchronize the labels in the input path
    #:  when creating an object detection or image classification dataset.
    #:  The default value is true.
    import_annotations = resource.Body('import_annotations', type=bool)
    #: Label format information. This parameter is used only when a
    #:  text classification dataset is created.
    ...
    #:  default.
    type = resource.Body('type', type=int)
    #: If no built-in or custom attribute needs to be set for labels,
    #:  this field can be omitted or left blank (the parameter value is {}).
    ...
    enterprise_project_id = resource.Body('enterprise_project_id', type=int)
