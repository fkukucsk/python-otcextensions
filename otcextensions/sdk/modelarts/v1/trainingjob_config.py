...
class DataSource(resource.Resource):
    ...
    #: Dataset type. Possible values are as follows: obs: Data from OBS is used. dataset:
    # Data from a specified dataset is used.
...
class NFS(resource.Resource):
    ...
    # true: read-only permission false: read/write permission. This is the default value.
...
class Volume(resource.Resource):
    #: Storage volume of the shared file system type. Only the training jobs running in the resource pool
    # with the shared file system network connected support such storage volume.
    nfs = resource.Body('nfs ', type=NFS)
    #: Storage volume of the host file system type. Only training jobs running in the dedicated
    # resource pool support such storage volume.
...
class TrainingjobConfig(resource.Resource):
    ...
    #: Running parameters of a training job. It is a collection of label-value pairs.
    # This parameter is a container environment variable when a job uses a custom image.
