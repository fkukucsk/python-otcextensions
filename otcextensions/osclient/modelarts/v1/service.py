    ...
    def take_action(self, parsed_args):
        ...
        for name_or_id in parsed_args.service:
            try:
                ...
                result += 1
                LOG.error(_("Failed to delete service(s) with "
                          "ID or Name '%(service)s': %(e)s"),
                          {'service': name_or_id, 'e': e})
        if result > 0:
            total = len(parsed_args.service)
            msg = (_("%(result)s of %(total)s Service(s) failed "
...
class UpdateConfiguration(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            '--description',
            metavar='<description>',
            help=_('Service description, which contains a maximum of 100 '
        ...
        parser.add_argument(
            '--status',
            metavar='<status>',
            help=_('Service status. The value can be running or stopped. '
        ...
        parser.add_argument(
            '--config',
            metavar='<config>',
            help=_('Service configuration. If this parameter is not set, '
                   'the service is not updated.')
        )
        parser.add_argument(
            '--schedule',
            metavar='<schedule>',
            help=_('Service scheduling configuration, which can be '
        ...
        parser.add_argument(
            ...
            help=_('Additional service attribute, which facilitates service '
                   'management.')
        )
        return parser
...
class CreateService(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'name',
            metavar='<name>',
            help=_('Service name. The value can contain 1 to 64 visible '
        ...
        parser.add_argument(
            ...
            required=False,
            help=_('ID of the workspace to which a service belongs. '
        ...
        parser.add_argument(
            ...
            dest='vpc_id',
            required=False,
            help=_('ID of the VPC to which a real-time service instance '
        ...
        parser.add_argument(
            ...
            required=False,
            help=_('ID of a subnet. By default, this parameter is left '
        ...
        parser.add_argument(
            ...
            required=False,
            help=_('A security group is a virtual firewall that provides '
        ...
        parser.add_argument(
            ...
            metavar='<cluster_id>',
            required=False,
            help=_('ID of a dedicated cluster. This parameter is left blank '
        ...
        parser.add_argument(
            ...
            metavar='<config>',
            required=True,
            help=_('Model running configuration. If infer_type is batch, '
        ...
        parser.add_argument(
            ...
            metavar='<weight>',
            required=False,
            help=_('Traffic weight allocated to a model. This parameter '
        ...
        parser.add_argument(
            ...
            help=_('Resource specifications. Select specifications based on '
        ...
        parser.add_argument(
            ...
            required=False,
            help=_('Number of instances deployed in a model The value must '
                   'be greater than 0.')
        )
        parser.add_argument(
            '--type',
            metavar='<type>',
            required=False,
            help=_('Scheduling type. Currently, only the value stop '
                   'is supported.')
        )
        parser.add_argument(
            '--time_unit',
            metavar='<time_unit>',
            required=False,
            help=_('Scheduling time unit. Possible values are as follows: '
                   'DAYS HOURS MINUTES')
        )
        parser.add_argument(
            '--duration',
            metavar='<duration>',
            required=False,
            help=_('Value that maps to the time unit. For example, '
...
class ListServices(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        table = (self.columns, (utils.get_dict_properties(
                                s, self.columns) for s in data))
        return table
