    ...
    def take_action(self, parsed_args):
        ...
        for name_or_id in parsed_args.model:
            try:
                ...
                result += 1
                LOG.error(_("Failed to delete model(s) with "
                          "ID or Name '%(model)s': %(e)s"),
                          {'model': name_or_id, 'e': e})
        if result > 0:
            total = len(parsed_args.model)
            msg = (_("%(result)s of %(total)s Model(s) failed "
...
class CreateModel(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'name',
            metavar='<name>',
            help=_('Model name. The value can contain 1 to 64 visible '
        ...
        parser.add_argument(
            ...
            required=True,
            help=_('OBS path where the model is located or the template '
                   'address of the SWR image.')
        )
        parser.add_argument(
            '--source-job-id',
            metavar='<source_job_id>',
            help=_('ID of the source training job. If the model is generated '
        ...
        parser.add_argument(
            ...
            help=_('Version of the source training job. If the model is '
        ...
        parser.add_argument(
            '--source-type',
            metavar='<source_type>',
            help=_('Model source type. Currently, the value can only be '
        ...
        parser.add_argument(
            ...
            metavar='<model_type>',
            required=True,
            help=_('Model type. The value can be TensorFlow, MXNet, Caffe, '
        ...
        parser.add_argument(
            '--runtime',
            metavar='<runtime>',
            help=_('Model running environment. The possible values of '
                   'runtime are related to model_type.')
        )
        parser.add_argument(
            '--description',
            metavar='<description>',
            help=_('Model remarks. The value contains a maximum of 100 '
        ...
        parser.add_argument(
            ...
            help=_('OBS path for storing the execution code. By default, '
        ...
        parser.add_argument(
            ...
            help=_('Collection of input parameters of a model. By default, '
                   'this parameter is left blank.')
        )
        parser.add_argument(
            '--output-params',
            metavar='<output_params>',
            help=_('Collection of output parameters of a model. By default, '
                   'this parameter is left blank.')
        )
        parser.add_argument(
            '--dependencies',
            metavar='<dependencies>',
            help=_('Package required for inference code and model. '
                   'By default, this parameter is left blank.')
        )
        parser.add_argument(
            '--model-algorithm',
            metavar='<model_algorithm>',
            help=_('Model algorithm. If the algorithm is read from the '
        ...
        parser.add_argument(
            ...
            help=_('Model precision, which is read from the '
                   'configuration file.')
        )
        parser.add_argument(
            '--apis',
        ...
        parser.add_argument(
            ...
            help=_('Character string converted from the final model '
        ...
        parser.add_argument(
            '--model-docs',
            metavar='<model_docs>',
            help=_('List of model description documents. A maximum of three '
...
class ListModels(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar='<limit>',
            type=int,
            help=_('Maximum number of records returned on each page. '
        ...
        parser.add_argument(
            '--order',
            metavar='<order>',
            help=_('Sorting order. The value can be asc or desc, '
        ...
        parser.add_argument(
            '--workspace-id',
            metavar='<workspace_id>',
            help=_('ID of the workspace to which a service belongs. The '
                   'default value is 0, indicating the default workspace.')
        )
        parser.add_argument(
            '--sort-by',
            metavar='<sort_by>',
            help=_('Sorting mode. The value can be create_at, model_'
        ...
        parser.add_argument(
            '--model-type',
            metavar='<model_type>',
            help=_('Model type. The models of this type are queried. '
        ...
        parser.add_argument(
            ...
            help=_('Model type. A list of models of types except '
