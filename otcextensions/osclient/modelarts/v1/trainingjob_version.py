...
_formatters = {
    'system_metric_list': modelarts_utils.YamlFormat
}
...
class CreateTrainingJobVersion(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            '--job-desc',
            metavar='<job_desc>',
            help=_('Description of a training job. The value is a string of '
        ...
        parser.add_argument(
            ...
            required=True,
            help=_('Boot file of a training job, which needs to be '
                   'stored in the code directory.')
        )
        parser.add_argument(
            '--parameter',
            metavar='<parameter>',
            help=_('Running parameters of a training job. '
...
class ListTrainingJobVersions(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        #    break
        table = (self.columns,
                 (utils.get_dict_properties(s, self.columns
                                            ) for s in data))
        return table
